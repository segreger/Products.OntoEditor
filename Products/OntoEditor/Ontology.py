...
schema = Schema((
    ...
    ReferenceField(
        ...
        widget=ReferenceBrowserWidget(
            label='internal_import',
            label_msgid='OntoEditor_label_internal_import',
            i18n_domain='OntoEditor',
        ),
        allowed_types=('Ontology',),
        multiValued=1,
        relationship='internal_import',
    ),
...
Ontology_schema = BaseFolderSchema.copy() + \
    schema.copy()

##code-section after-schema #fill in your manual code here
##/code-section after-schema

class Ontology(BaseFolder, BrowserDefaultMixin):
    """
    """
    security = ClassSecurityInfo()

    implements(interfaces.IOntology)
    ...
    def getPath(self):
        ...
        security.declarePublic('getObjectsByType')
    ...
    security.declarePublic('getObjectsByName')
    ...
    def getClasses(self):
        ...
        """
        # получаем все классы
        res=self.getObjectsByType( 'OntoClass' )
        ontolist=self.getInternal_import()
        #pdb.set_trace()
        if ontolist:
            for onto in ontolist:
                res=res+onto.getClasses()
        return res
    security.declarePublic('getRootClass')
    def getRootClass(self):
        ...
        root={}
        for o in res:
            if 'rootclass' in o.__dict__.keys():
                   if o.rootclass:
                       root['title']=o.title_or_id()
                       root['url']=o.absolute_url()
        if root:
            return root
        else:
            return None
